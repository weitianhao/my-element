...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { emit }) {
    ...
    const steps$1 = vue.ref([]);
    const current = core.useVModel(props, "current", emit, {
      passive: true
    });
    const total = vue.computed(() => steps$1.value.length);
    const currentStep = vue.computed(() => steps$1.value[current.value] || {});
    const currentTarget = vue.computed(() => currentStep.value.target);
    const kls = vue.computed(() => [
      ns.b(),
      mergedType.value === "primary" ? ns.m("primary") : ""
    ]);
    const mergedPlacement = vue.computed(() => currentStep.value.placement || props.placement);
